...
const EngButton = document.querySelector(".eng-lang");
...
async function getQuotesEng() {
  const quotes = "lang_english_quotes.json";
  const res = await fetch(quotes);
  const data = await res.json();
  let min = Math.ceil(0);
...
let changeQuote = () => {
  ...
  if (lang == "bel") {
    ...
    getQuotesEng();
    getQuotesEng().then(function (result) {
      heroAuthor.textContent = `As ${result.author} said`;
      heroQuote.textContent = result.text;
      heroButton.textContent = "Next";
...
let getTranslateLang = (event) => {
  ...
  if (event.target.classList.contains("bel-lang")) {
    ...
    event.target.classList.toggle("active");
    EngButton.classList.toggle("active");
    getTranslate(language);
  }
};
